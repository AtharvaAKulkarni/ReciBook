...
export const Recipe = () => {
  ...
  useEffect(() => {
    const getRecipeInfo = async () => {
      try {
        const res = await axios.post(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/get-recipes`, {
          condition: "id",
          id,
        });
  ...
  return (
    <div className="flex flex-col h-full w-full bg-green-950">
      ...
      <div className="flex gap-2 w-fit h-fit mx-auto">
        ...
        <img
          src={`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}${recipe?.imageUrl}`}
          alt="Recipe"
      ...
      <section className="bg-amber-50 mt-8" id="recipe">
        <div className="bg-purple-400 flex flex-col mt-13 w-[50vw] mx-auto mb-8 p-1 pb-3">
          <img src={`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}${recipe?.imageUrl}`} className="h-25 w-25 rounded-full mx-auto -translate-y-8" />
