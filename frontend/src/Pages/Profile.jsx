...
export const Profile = () => {
    ...
    useEffect(() => {
        ...
        const getProfile = async () => {
            try {
                ...
                const response = await axios.post(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/get-profile`, { userId: user.id },
                    {
                        headers: {
    ...
    useEffect(() => {
        const getRecipes = async () => {
            try {
                const response = await axios.post(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/get-recipes`, { condition: "user", id: profile.name});
    ...
    const handleProfilePictureUpdate = async (event) => {
        ...
        try {
            const response = await axios.put(
                `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/update-profile/picture`,
                formData,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            if (response.data.success) {
                ...
                setProfile({ ...profile, profilePicture: `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/${response.data.user.profilePicture}` }); // Update UI
            }
        } catch (error) {
    ...
    const handleBio = async () => {
        ...
        try {
            const response = await axios.put(
                `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/update-profile/bio`,
                            ...
                            recipes.map((recipe, index) => (
                                <Link to={`/recipe/${recipe._id}`}>
                                    <div className=' w-[80%] p-3 flex gap-4 hover:bg-white group transition-[10s] ease-in-out rounded-2xl'>
                                        <img className='w-40 h-40 ml-10 group-hover:scale-110 rounded-2xl' src={`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}${recipe?.imageUrl}`} />
